				...
				Person.find({name:req.session.now_user.id},function(err,response1){
					postTip.find((err,response)=>{
						...
						if(response1[0]['is_admin']==1){
							res.render('adminhome',{postSuccess:"发帖成功！",is_login:true,id:req.session.now_user.id,is_active1:"active",resq:arr,isTable:true,lastpage:1,nextpage:2,img:response1[0]['headPic'],active_all:"active",whattype:"",is_admin:true});
						}else{
							res.render('home',{postSuccess:"发帖成功！",is_login:true,id:req.session.now_user.id,is_active1:"active",resq:arr,isTable:true,lastpage:1,nextpage:2,img:response1[0]['headPic'],active_all:"active",whattype:""});
						}
					})
				})
			}
